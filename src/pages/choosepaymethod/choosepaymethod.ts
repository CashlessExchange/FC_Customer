...
@IonicPage()
@Component({
  selector: 'page-choosepaymethod',
  templateUrl: 'choosepaymethod.html',
})
export class ChoosepaymethodPage {
    ...
    if(param!="solo"){
      if(this.price!="0.00"){
        console.log(this.feeForMerchant);
            ...
            await this.calcFee(this.items.price);
        
            console.log(String(Number(this.items.price).toFixed(2) + this.feeForMerchant));
            let calculated = Number(Number(this.items.price).toFixed(2)) + this.feeForMerchant;
  ...
  async calcFee(ammount: string) {
    ...
    let value = Number(ammount) * (Number(datas.fees.processing_fees)/100);
    ...
    if(this.selectedCard != undefined){
     ...
     if(!Number.isInteger(last4)){
      ...
      if(this.selectedCard.cardname === "Cash/Check"){
        console.log("cash");
        this.navCtrl.push(CheckoutPage, { merchant: this.items.merchantid, price: newPrice, item: this.items.name, deal: this.items.deal, ref: this.items.ref, selectedcard:this.selectedCard ,discount:discount, fees:this.feeForMerchant, vaucherid:this.vaucherid, certs:this.chosenToken, guthaben:this.guthaben ,itemsprice:this.items.price});
        return;
      }else{
